...
safe_get <- function(url, accept='*/*') tryCatch({
  resp <- request(url) |>
    ...
    req_user_agent('fulltexttest/1.0 reproducible research retrieval') |>
    req_timeout(30) |>
    req_retry(max_tries=2) |>
    req_perform()
  list(ok=resp_status(resp)>=200&&resp_status(resp)<300,
       status=resp_status(resp),
       content_type=resp_headers(resp)[['content-type']]%||%'',
...
attempt_row <- function(id,route,url,res,candidate='',note='') data.frame(
  record_number=id,route=route,url=url,status=res$status,content_type=res$content_type,
  usable_full_text=isTRUE(res$ok&&is_doc(res$content_type,res$bytes)),candidate_url=candidate,
  note=note,error=res$error,stringsAsFactors=FALSE)
...
for(i in seq_len(nrow(b))){
  ...
  if(!nzchar(doi)){
    ...
    if(res$ok){j<-tryCatch(fromJSON(rawToChar(res$bytes)),error=function(e)NULL);items<-tryCatch(j$message$items,error=function(e)NULL);if(!is.null(items)&&nrow(items)>0)doi<-as.character(items$DOI[1]%||%'')}
  }
