...
def request(method,url,**kwargs):
 ...
 for attempt in range(4):
  ...
  log(f'HTTP 429; backing off {delay:.0f}s; remaining={r.headers.get("X-RateLimit-Remaining","?")} reset={r.headers.get("X-RateLimit-Reset","?")} credits={r.headers.get("X-RateLimit-Credits-Used","?")}');time.sleep(delay)
 return None
def parse_pdf(data):
...
def store(input_doi,work,source,url,data,ct):
 raw=gzip.decompress(data) if data[:2]==b'\x1f\x8b' else data;ct=(ct or '').lower()
 try:
...
state=json.loads(STATE.read_text()) if STATE.exists() else {'completed':{},'failed':{}};completed=set(state.get('completed',{}));rows=[]
