...
session.headers['User-Agent'] = 'fulltexttest/openalex-bulk-pdf/4.0'
...
def store(input_doi, work, source, url, data, content_type):
    ...
    (dest/'fulltext.'+ext).write_bytes(stored) if False else (dest/f'fulltext.{ext}').write_bytes(stored)
...
def download_pdf(input_doi, work, url):
    if not url or not str(url).startswith('http'): return None,'invalid_url'
    r=request('GET',url)
    if r is None:return None,'request_failed'
    if not r.ok or not r.content:return None,f'http_{r.status_code}'
    return store(input_doi,work,'OpenAlex-discovered OA PDF',r.url,r.content,r.headers.get('content-type',''))
...
successes=[]; failures=[]; start=time.time()
for start_i in range(0,len(rows),BATCH_SIZE):
    ...
    if r is None or not r.ok:
        ...
        failures.extend({'input_doi':d,'status':status} for d in batch)
        continue
    ...
    for n,input_doi in enumerate(batch, start_i+1):
        ...
        for pdf_url in urls:
            ...
            got,error=download_pdf(input_doi,work,pdf_url)
            if got:break
        if got:
...
for item in failures:state.setdefault('failed',{})[item['input_doi']]={'date':batch_date,'status':item['status']}
STATE.write_text(json.dumps(state,indent=2,ensure_ascii=False),encoding='utf-8');Path('daily-results.json').write_text(json.dumps({'date':batch_date,'run_id':run_id,'fulltexts_archived':len(successes),'failures':failures,'zenodo':{'record_id':published.get('id'),'doi':published.get('doi')}},indent=2,ensure_ascii=False),encoding='utf-8');log('CHECKPOINT STATE WRITTEN AFTER ZENODO PUBLICATION')