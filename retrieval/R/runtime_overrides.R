...
# system2() arguments are passed directly. URLs are normalised before retrieval.
...
safe_request <- function(url, timeout_seconds = 120L) {
  ...
  args <- c("-L", "--fail-with-body", "--silent", "--show-error",
            ...
            "--retry", "2", "--retry-delay", "2", "--user-agent", "fulltexttest-clean-r/4.3",
            "-D", hdr, "-o", tmp, url)
...
discover_search_urls <- function(row, timeout_seconds = 30L) {
  ...
  raw_json <- function(url, ext = ".json") {
    ...
    args <- c("-L", "--fail", "--silent", "--show-error", "--max-time", as.character(timeout_seconds),
              ...
              "--user-agent", "fulltexttest-clean-r/4.3", "-o", tmp, url)
    status <- suppressWarnings(system2("curl", args, stdout = FALSE, stderr = FALSE))
