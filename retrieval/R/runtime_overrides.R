...
# Loaded last by run.R so these helpers override the broader discovery layer.
# No shell interpolation is used: every curl argument is passed directly to
# system2(). This is important for URLs containing parentheses, ampersands,
...
safe_request <- function(url, timeout_seconds = 120L) {
  ...
  args <- c('-L', '--fail-with-body', '--silent', '--show-error',
            '--max-time', as.character(timeout_seconds), '--connect-timeout', '20',
            '--retry', '3', '--retry-delay', '4', '--retry-max-time', '45',
            '--retry-all-errors', '--user-agent', 'fulltexttest-clean-r/4.3',
            '-D', hdr, '-o', tmp, url)
...
# Override the discovery layer's HTTP helper too. This removes the last route
# by which an unnormalised candidate could reach curl.
.discovery_curl <- function(url, timeout_seconds = 20L, tries = 2L) {
  r <- safe_request(url, min(as.integer(timeout_seconds), 60L))
  if (isTRUE(r$ok) && length(r$body)) {
    return(tryCatch(rawToChar(r$body), error = function(e) ''))
  }
  ''
}

# Override discovery with a rate-limit-aware implementation. Search engines are
# used for candidate discovery; APIs are queried only when a DOI exists.
discover_search_urls <- function(row, timeout_seconds = 30L) {
  ...
  out <- character()
  queries <- c(paste0('"', title, '" filetype:pdf'),
  ...
  for (q in queries) {
    ...
    if (nzchar(ddg)) {
      hits <- regmatches(ddg, gregexpr('https?://[^\"\'<>[:space:]]+', ddg, perl = TRUE))[[1]]
      if (length(hits)) out <- c(out, hits)
      hrefs <- regmatches(ddg, gregexpr('uddg=https?[^&"<> ]+', ddg, perl = TRUE))[[1]]
      if (length(hrefs)) out <- c(out, utils::URLdecode(sub('^.*uddg=', '', hrefs)))
    }
    Sys.sleep(2)
  }

  if (nzchar(doi)) {
    oa <- .discovery_curl(paste0('https://api.openalex.org/works/https://doi.org/',
                                 utils::URLencode(doi, reserved = TRUE),
                                 '?mailto=fulltexttest@example.org'), min(timeout_seconds, 25L))
    if (nzchar(oa)) out <- c(out, json_urls(oa, c('best_oa_location', 'pdf_url', 'landing_page_url', 'url_for_pdf', 'url')))
