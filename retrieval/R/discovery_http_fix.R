...
# Sourced after additive_discovery_v3.R so discovery requests use the
# container's actual curl executable and preserve HTTP status as data.
.disc_request <- function(url, timeout = 25L) {
  ...
  body_file <- tempfile(fileext=".bin")
  header_file <- tempfile(fileext=".headers")
  meta_file <- tempfile(fileext=".meta")
  ...
  args <- c(
    ...
    "--connect-timeout", "10",
    "--user-agent", "Mozilla/5.0 fulltexttest-discovery/5.1",
    "-D", header_file,
    "-o", body_file,
  ...
  status <- NA_integer_
  if (nzchar(meta)) {
    bits <- strsplit(meta, "\\t", fixed=FALSE)[[1]]
    if (length(bits) >= 1L) status <- suppressWarnings(as.integer(bits[[1]]))
    if (length(bits) >= 2L && nzchar(bits[[2]])) effective <- bits[[2]]
  }

  # HTTP status is authoritative when available. Use the last response block
  # as a fallback because redirects produce multiple HTTP header sections.
  if (is.na(status)) {
  ...
  err <- if (exit != 0L) {
    paste0("curl_exit_", exit, if (nzchar(errtxt)) paste0(":", errtxt) else "")
  } else ""

  list(
    ok=!is.na(status) && status >= 200L && status < 300L,
    status=status,
    type=type,
    body=body,
    final_url=effective,
