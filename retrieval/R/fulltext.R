...
candidate_urls <- function(row) {
  ...
  out <- character()
  if (!is.null(urls)) out <- c(out, row[[urls]])
  if (!is.null(doi) && nzchar(row[[doi]])) {
    d <- row[[doi]]
    if (!grepl("^https?://", d, ignore.case = TRUE)) d <- paste0("https://doi.org/", d)
    out <- c(out, d)
  }
  unique(out[nzchar(out)])
}

safe_request <- function(url, timeout_seconds = 20L) {
  started <- Sys.time()
  res <- tryCatch(
    {
      h <- curl::new_handle(useragent = "fulltexttest-clean-r/1.0", timeout = timeout_seconds, connecttimeout = min(10L, timeout_seconds))
      req <- curl::curl_fetch_memory(url, handle = h)
      list(ok = TRUE, status = req$status_code, type = req$type %||% "", body = req$content, error = "", elapsed = as.numeric(difftime(Sys.time(), started, units = "secs")))
    },
    error = function(e) list(ok = FALSE, status = NA_integer_, type = "", body = raw(), error = conditionMessage(e), elapsed = as.numeric(difftime(Sys.time(), started, units = "secs")))
  )
  res
}
...
run_one <- function(row, out_dir, timeout_seconds = 20L) {
  nms <- names(row)
  rid_col <- find_first_column(nms, c("record_id", "id"))
  rid <- row[[rid_col]]
  ...
  for (u in urls) {
    ...
    v <- if (r$ok && r$status >= 200 && r$status < 300) parse_response(r$body, r$type, u) else list(ok = FALSE, chars = 0L, reference_markers = 0L, reason = paste0("http_", r$status), format = "unknown", extension = ".bin", parsed_text = "")
    attempts[[length(attempts) + 1L]] <- data.frame(record_id = rid, url = u, status = r$status, content_type = r$type, bytes = length(r$body), verified_complete = isTRUE(v$ok), format = v$format, reason = v$reason, elapsed_seconds = r$elapsed, stringsAsFactors = FALSE)
    if (isTRUE(v$ok)) {
      winner <- list(url = u, response = r, validation = v)
      break
    }
  }

  audit <- if (length(attempts)) do.call(rbind, attempts) else data.frame(record_id = rid, url = NA_character_, status = NA_integer_, content_type = "", bytes = 0L, verified_complete = FALSE, format = "", reason = "no_candidate_urls", elapsed_seconds = 0, stringsAsFactors = FALSE)
  ...
  if (!is.null(winner)) {
    ...
    status <- data.frame(record_id = rid, full_text_status = "verified_complete", format = winner$validation$format, source_url = winner$url, text_chars = winner$validation$chars, reference_markers = winner$validation$reference_markers, stringsAsFactors = FALSE)
  } else {
    status <- data.frame(record_id = rid, full_text_status = "unobtainable", format = "", source_url = "", text_chars = 0L, reference_markers = 0L, stringsAsFactors = FALSE)
  }
