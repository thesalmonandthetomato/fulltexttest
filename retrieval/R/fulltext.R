# v8 fulltext baseline: curl only for document retrieval.
...
safe_request <- function(url, timeout_seconds=30L) {
  ...
  args <- c(
    ...
    "--connect-timeout", "15",
    "--user-agent", "fulltexttest-clean-r/8.0",
    "-D", hdr, "-o", tmp,
  ...
  if (is.na(code)) {
    ...
    if (length(status_line)) {
      code <- suppressWarnings(as.integer(sub(
        "^HTTP/[0-9.]+\\s+([0-9]+).*$", "\\1", status_line
      )))
    }
  }

  ct_lines <- grep("^Content-Type:", headers, ignore.case=TRUE, value=TRUE)
  content_type <- if (length(ct_lines)) {
    trimws(sub("^Content-Type:\\s*", "", tail(ct_lines, 1L), ignore.case=TRUE))
  } else ""

  errtxt <- if (file.exists(errf)) paste(readLines(errf, warn=FALSE), collapse=" ") else ""
  error <- if (exit_code != 0L) {
    paste0("curl_exit_", exit_code, if (nzchar(errtxt)) paste0(":", errtxt) else "")
  } else ""

  list(
    ok=!is.na(code) && code >= 200L && code < 300L && length(body) > 0L,
...
run_one <- function(row, out_dir, timeout_seconds=30L) {
  ...
  for (u in urls) {
    ...
    v <- if (r$ok) parse_response(r$body, r$type, r$final_url) else list(
      ok=FALSE, chars=0L, reference_markers=0L,
      reason=if (is.na(r$status)) paste0("request_error: ", r$error) else paste0("http_", r$status),
      format="unknown", extension=".bin", parsed_text=""
    )

    attempts[[length(attempts)+1L]] <- data.frame(
      record_id=rid, candidate_url=u, final_url=r$final_url, status=r$status,
      content_type=r$type, bytes=length(r$body), verified_complete=isTRUE(v$ok),
      format=v$format, reason=v$reason, elapsed_seconds=r$elapsed,
  ...
  audit <- if (length(attempts)) do.call(rbind, attempts) else data.frame(
    record_id=rid, candidate_url=NA_character_, final_url=NA_character_, status=NA_integer_,
    content_type="", bytes=0L, verified_complete=FALSE, format="", reason="no_candidate_urls",
  ...
  if (!is.null(winner)) {
    ...
    data.frame(
      record_id=rid, full_text_status="verified_complete", format=winner$validation$format,
    ...
    data.frame(
      record_id=rid, full_text_status="unobtainable", format="", source_url="",
      text_chars=0L, reference_markers=0L, stringsAsFactors=FALSE
    )
  }
}
